...
# ═════════════════════════════════════════════════════════════════
# LIVE DASHBOARD — lightweight web server
# Serves a Bloomberg-style HTML dashboard at PORT (default 8080)
# Railway exposes this automatically — just click the generated URL
# ═════════════════════════════════════════════════════════════════

def build_dashboard_html() -> str:
    """Generate live HTML dashboard from current bot state."""
    ...
    bal_col  = "#00d17a" if bal >= 500 else "#ff4757"
    macro    = state.get("macro_context", {})
    open_pos = state.get("open_positions", {})
    thresh   = get_score_threshold()

    # Recent trades
    recent = trade_log[-20:] if trade_log else []
    recent.reverse()
    trade_rows = ""
    for t in recent:
        pnl_t   = t.get("pnl", 0)
        pc      = "#00d17a" if pnl_t >= 0 else "#ff4757"
        ic      = "✅" if t.get("result") == "WIN" else "❌"
        ts      = t.get("timestamp", "")[:16].replace("T"," ")
        trade_rows += f"""<tr>
            <td>{ic}</td>
            <td>{t.get('symbol','?')}</td>
            <td style="color:{'#00d17a' if t.get('side')=='long' else '#ff4757'};font-weight:700">{t.get('side','?').upper()}</td>
            <td>${t.get('entry',0):,.2f}</td>
            <td>${t.get('exit',0):,.2f}</td>
            <td style="color:{pc};font-weight:700">${pnl_t:+.2f}</td>
            <td>{t.get('score',0)}/100</td>
            <td>{ts}</td>
        </tr>"""
    ...
    pos_rows = ""
    for sym, pos in open_pos.items():
        side_col = "#00d17a" if pos["side"] == "long" else "#ff4757"
        pos_rows += f"""<tr>
            <td>{sym}</td>
            <td style="color:{side_col};font-weight:700">{pos['side'].upper()}</td>
            <td>${pos['entry']:,.4f}</td>
            <td>${pos['stop']:,.4f}</td>
            <td>${pos['target']:,.4f}</td>
            <td>{pos.get('score',0)}/100</td>
            <td>{pos.get('rr',0):.1f}:1</td>
        </tr>"""

    if not pos_rows:
        pos_rows = '<tr><td colspan="7" style="text-align:center;color:#475569;padding:20px">No open positions</td></tr>'

    if not trade_rows:
        trade_rows = '<tr><td colspan="8" style="text-align:center;color:#475569;padding:20px">No trades yet</td></tr>'

    # Macro pills
    macro_pills = ""
    for k, v in macro.items():
        if k == "signal_strength": continue
        col = "#00d17a" if v in ("dovish","falling","weakening","rising") else "#ff4757" if v in ("hawkish","strong","positive") else "#ffb800"
        macro_pills += f'<span style="background:{col}22;color:{col};border:1px solid {col}44;padding:3px 12px;border-radius:20px;font-size:11px;margin:3px;display:inline-block">{k.replace("_"," ").title()}: {v}</span>'
    ...
    for sym, intel in asset_intel.items():
        ...
        color = ASSETS[sym]["color"]
        bar_w = int(heat)
        bc = "#00d17a" if heat>=70 else "#ffb800" if heat>=50 else "#ff4757"
        pred_icon = "📈" if pred=="bull" else "📉" if pred=="bear" else "➡️"
        heat_rows += f"""<tr>
            <td>{color} {ASSETS[sym]['label']}</td>
            <td><div style="background:#1a2130;border-radius:3px;height:8px;width:100px">
                <div style="background:{bc};width:{bar_w}%;height:8px;border-radius:3px"></div>
            </div> {heat:.0f}</td>
            <td>{pred_icon} {pred} ({conf}%)</td>
            <td style="color:{bc}">{mult:.1f}x</td>
        </tr>"""
...
<title>Meridian Trading Bot</title>
<link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;500;600;700&display=swap" rel="stylesheet">
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
body{{background:#08090b;color:#e2e8f0;font-family:'IBM Plex Mono',monospace;font-size:12px}}
.header{{background:#0e1117;border-bottom:1px solid rgba(255,255,255,.07);padding:12px 20px;display:flex;align-items:center;justify-content:space-between}}
.logo{{font-size:16px;font-weight:700;color:#ff6b00;letter-spacing:-.02em}}
.live{{display:flex;align-items:center;gap:6px;font-size:10px;color:#475569}}
.dot{{width:7px;height:7px;border-radius:50%;background:#00d17a;animation:pulse 2s infinite}}
@keyframes pulse{{0%,100%{{opacity:1}}50%{{opacity:.3}}}}
.grid{{display:grid;grid-template-columns:repeat(5,1fr);gap:1px;background:rgba(255,255,255,.05);border-bottom:1px solid rgba(255,255,255,.07)}}
.stat{{background:#0e1117;padding:14px 16px}}
.sl{{font-size:9px;color:#475569;text-transform:uppercase;letter-spacing:.1em;margin-bottom:6px}}
.sv{{font-size:20px;font-weight:700}}
.body{{padding:16px;display:grid;grid-template-columns:1fr 1fr;gap:16px}}
.card{{background:#0e1117;border:1px solid rgba(255,255,255,.07);border-radius:6px;overflow:hidden}}
.ch{{padding:10px 14px;border-bottom:1px solid rgba(255,255,255,.07);font-size:10px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.08em}}
table{{width:100%;border-collapse:collapse;font-size:11px}}
th{{padding:7px 12px;text-align:left;color:#475569;font-size:9px;text-transform:uppercase;letter-spacing:.07em;background:#141920;border-bottom:1px solid rgba(255,255,255,.07)}}
td{{padding:7px 12px;border-bottom:1px solid rgba(255,255,255,.04);color:#94a3b8}}
tr:last-child td{{border-bottom:none}}
tr:hover td{{background:#141920}}
.macro{{padding:12px 14px}}
.footer{{padding:8px 20px;text-align:center;font-size:9px;color:#2d3748;border-top:1px solid rgba(255,255,255,.04)}}
.mode-badge{{background:rgba(255,184,0,.1);border:1px solid rgba(255,184,0,.25);color:#ffb800;padding:3px 10px;border-radius:3px;font-size:9px;font-weight:700}}
@media(max-width:768px){{.grid{{grid-template-columns:repeat(2,1fr)}}.body{{grid-template-columns:1fr}}}}
</style>
</head>
<body>
<div class="header">
  ...
  <div class="live"><div class="dot"></div>Live · refreshes every 30s · {now}</div>
</div>
...
<div class="body">
  <div class="card">
    ...
    <table><thead><tr><th>Asset</th><th>Side</th><th>Entry</th><th>Stop</th><th>Target</th><th>Score</th><th>R:R</th></tr></thead>
    <tbody>{pos_rows}</tbody></table>
  </div>

  <div class="card">
    <div class="ch">Asset Intelligence</div>
    <table><thead><tr><th>Asset</th><th>Heat</th><th>Prediction</th><th>Allocation</th></tr></thead>
    <tbody>{heat_rows}</tbody></table>
  </div>

  <div class="card" style="grid-column:1/-1">
    <div class="ch">Macro Context</div>
    <div class="macro">{macro_pills if macro_pills else '<span style="color:#475569">Loading macro data...</span>'}</div>
  </div>

  <div class="card" style="grid-column:1/-1">
    <div class="ch">Recent Trades</div>
    <table><thead><tr><th></th><th>Asset</th><th>Side</th><th>Entry</th><th>Exit</th><th>P&L</th><th>Score</th><th>Time</th></tr></thead>
    <tbody>{trade_rows}</tbody></table>
  </div>
</div>

<div class="footer">Meridian Trading Bot v2.0 · {mode} · Balance ${bal:,.2f} · {now}</div>
</body></html>"""
